...
Intended to be used as the final preprocessing step. This function
creates data epochs of either fixed or dynamic durations with respect to
provided `events` and time `limits`, and also includes an intuitive
metadata parsing feature where additional trial data embedded within
event messages can easily be identified and joined into the resulting
epoched data frames.
...
- events:
  ...
  Either (1) a single string representing the event message to perform
  trial extraction around, using specified `limits` to center the epoch
  around or no `limits` (which then just grabs the data epochs between
  each subsequent event string of the same type); (2) a vector
  containing both `start` and `end` event message strings – here,
  `limits` will be ignored and the duration of each trial epoch will be
  the number of samples between each matched `start` and `end` event
  message pair; or (3) a list of 2 data frames that manually specify
  start/end event timestamp-message pairs to pull out of the raw time
  series data – here, it is required that each raw timestamp and event
  message be provided in the following format:

  list( data.frame(time = c(...), msg = c(...)), \# start events
  data.frame(time = c(...), msg = c(...)), \# end events 1 \# block
  number )

  where the first data.frame indicates the `start` event timestamp and
  message string pairs, and the second data.frame indicates the `end`
  event timestamp and message string pairs. Additionally, manual
  epoching only words with 1 block at a time for event-modes `2` and
  `3`; thus, please be sure to explicitly indicate the block number in
  your input list (for examples, see above as well as example \#9 below
  for more details)

  For event-modes `1` and `2`, the way in which you pass in the event
  message string must conform to a standardized protocol so that
  `eyeris` knows how to find your events and (optionally) parse any
  included metadata into the tidy epoch data outputs. You have two
  primary choices: either (a) specify a string followed by a `*`
  wildcard expression (e.g., `"PROBE_START*`), which will match any
  messages that have "PROBE_START ..." (... referring to potential
  metadata, such as trial number, stim file, etc.); or (b) specify a
  string using the `eyeris` syntax: (e.g., `"PROBE_{type}_{trial}"`),
  which will match the messages that follow a structure like this
  "PROBE_START_1" and "PROBE_STOP_1", and generate two additional
  metadata columns: `type` and `trial`, which would contain the
  following values based on these two example strings: `type`:
  `('START', 'STOP')`, and `trial`: `(1, 1)`

- limits:

  A vector of 2 values (start, end) in seconds, indicating where trial
  extraction should occur centered around any given `start` message
  string in the `events` parameter

- label:

  An (optional) string you can provide to customize the name of the
  resulting `eyeris` class object containing the epoched data frame. If
  left as `NULL` (default), then list item will be called `epoch_xyz`,
  where `xyz` will be a sanitized version of the original `start` event
  string you provided for matching. If you choose to specify a `label`
  here, then the resulting list object name will take the form:
  `epoch_label`. **Warning: if no `label` is specified and there are no
  event message strings to sanitize, then you may obtain a
  strange-looking epoch list element in your output object (e.g.,
  `$epoch_`, or `$epoch_nana`, etc.). The data should still be
  accessible within this nested lists, however, to avoid ambiguous list
  objects, we recommend you provide an `epoch` label here to be safe**

- baseline:

  **(New)** A single parameter that controls baseline correction. Set to
  `TRUE` to both calculate and apply baseline correction, or `FALSE` to
  skip it. This replaces the deprecated `calc_baseline` and
  `apply_baseline` parameters

- baseline_type:

  Whether to perform *subtractive* (`sub`) or *divisive* (`div`)
  baseline correction. Defaults to `sub`

- baseline_events:

  Similar to `events`, `baseline_events`, you can supply either (1) a
  single string representing the event message to center the baseline
  calculation around, as indicated by `baseline_period`; or (2) a single
  vector containing both a `start` and an `end` event message string –
  here, `baseline_period` will be ignored and the duration of each
  baseline period that the mean will be calculated on will be the number
  of samples between each matched `start` and `end` event message pair,
  as opposed to a specified fixed duration (as described in 1). Please
  note, providing a list of trial-level start/end message pairs (like in
  the `events` parameter) to manually indicate unique start/end chunks
  for baselining is currently unsupported. Though, we intend to add this
  feature in a later version of `eyeris`, given it likely won't be a
  heavily utilized / in demand feature.

- baseline_period:

  A vector of 2 values (start, end) in seconds, indicating the window of
  data that will be used to perform the baseline correction, which will
  be centered around the single string "start" message string provided
  in `baseline_events`. Again, `baseline_period` will be ignored if both
  a "start" **and** "end" message string are provided to the
  `baseline_events` argument

- hz:

  Data sampling rate. If not specified, will use the value contained
  within the tracker's metadata

- verbose:

  A flag to indicate whether to print detailed logging messages Defaults
  to `TRUE`. Set to `False` to suppress messages about the current
...
An `eyeris` object with a new nested list of data frames: `$epoch_*`.
The epochs are organized hierarchically by block and preprocessing step.
Each epoch contains the pupil time series data for the specified time
window around each event message, along with metadata about the event.

When using
[`bidsify()`](https://eyeris.shawnschwartz.com/reference/bidsify.md) to
export the data, filenames will include both epoch and baseline event
...
#> ✔ [2026-07-03 18:40:59] [OKAY] Running eyeris::load_asc()
#> ℹ [2026-07-03 18:40:59] [INFO] Processing block: block_1
#> ✔ [2026-07-03 18:40:59] [OKAY] Running eyeris::deblink() for block_1
#> ✔ [2026-07-03 18:40:59] [OKAY] Running eyeris::detransient() for block_1
#> ✔ [2026-07-03 18:40:59] [OKAY] Running eyeris::interpolate() for block_1
#> ✔ [2026-07-03 18:40:59] [OKAY] Running eyeris::lpfilt() for block_1

#> ! [2026-07-03 18:40:59] [WARN] Skipping eyeris::downsample() for block_1
#> ! [2026-07-03 18:40:59] [WARN] Skipping eyeris::bin() for block_1
#> ! [2026-07-03 18:40:59] [WARN] Skipping eyeris::detrend() for block_1
#> ✔ [2026-07-03 18:40:59] [OKAY] Running eyeris::zscore() for block_1
#> ℹ [2026-07-03 18:40:59] [INFO] Block processing summary:
#> ℹ [2026-07-03 18:40:59] [INFO] block_1: OK (steps: 6, latest:
#> pupil_raw_deblink_detransient_interpolate_lpfilt_z)
#> ✔ [2026-07-03 18:40:59] [OKAY] Running eyeris::summarize_confounds()

# example 1: select 1 second before/after matched event message "PROBE*"
eye_preproc |>
  eyeris::epoch(events = "PROBE*", limits = c(-1, 1))
#> ℹ [2026-07-03 18:40:59] [INFO] Epoching pupil data...
#> ℹ [2026-07-03 18:40:59] [INFO] Block 1: found 10 matching events for PROBE
#> ✔ [2026-07-03 18:40:59] [OKAY] Done!
#> ✔ [2026-07-03 18:40:59] [OKAY] Block 1: pupil data from 10 unique event
#> messages extracted
#> ✔ [2026-07-03 18:40:59] [OKAY] Pupil epoching completed in 0.12 seconds
#> ℹ [2026-07-03 18:40:59] [INFO] Recalculating epoched confounds for new
#> epochs...
#> $file
...
#> 
#> $params$epoch
#> $params$epoch$call_stack
#> eyeris::epoch(eyeris = eye_preproc, events = "PROBE*", limits = c(-1, 
#>     1))
#> 
#> $params$epoch$parameters
#> $params$epoch$parameters$events
#> [1] "PROBE*"
#> 
#> $params$epoch$parameters$limits
#> [1] -1  1
#> 
#> $params$epoch$parameters$label
#> NULL
#> 
#> $params$epoch$parameters$baseline
#> [1] FALSE
#> 
#> $params$epoch$parameters$baseline_type
#> [1] "sub" "div"
#> 
#> $params$epoch$parameters$baseline_events
#> NULL
#> 
#> $params$epoch$parameters$baseline_period
#> NULL
#> 
#> $params$epoch$parameters$hz
#> NULL
#> 
#> $params$epoch$parameters$verbose
#> [1] TRUE
#> 
#> 
#> 
...
#> 
#> 
#> $confounds$epoched_timeseries
#> $confounds$epoched_timeseries$epoch_probe
#> $confounds$epoched_timeseries$epoch_probe$block_1
#>                                                                    matched_event
...
#> 
#> 
#> $confounds$epoched_epoch_wide
#> $confounds$epoched_epoch_wide$epoch_probe
#> $confounds$epoched_epoch_wide$epoch_probe$block_1
...
#> 
#> $epoch_probe$info
#> $epoch_probe$info$block_1
#> $epoch_probe$info$block_1$calc_baseline
#> [1] FALSE
#> 
#> $epoch_probe$info$block_1$apply_baseline
#> [1] FALSE
#> 
#> $epoch_probe$info$block_1$epoch_events
#> [1] "PROBE*"
#> 
#> $epoch_probe$info$block_1$epoch_limits
#> [1] -1  1
#> 
#> $epoch_probe$info$block_1$n_epochs
#> [1] 10
#> 
#> 
#> 
...
#> ℹ [2026-07-03 18:40:59] [INFO] Epoching pupil data...
#> ℹ [2026-07-03 18:40:59] [INFO] Block 1: found 5 matching events for
#> TRIALIDtrial
#> ✔ [2026-07-03 18:40:59] [OKAY] Done!
#> ✔ [2026-07-03 18:40:59] [OKAY] Block 1: pupil data from 5 unique event messages
#> extracted
#> ✔ [2026-07-03 18:40:59] [OKAY] Pupil epoching completed in 0.07 seconds
#> ℹ [2026-07-03 18:40:59] [INFO] Recalculating epoched confounds for new
#> epochs...
#> $file
#> [1] "/home/runner/work/_temp/Library/eyeris/extdata/memory.asc"
...
#> $params$epoch
#> $params$epoch$call_stack
#> eyeris::epoch(eyeris = eye_preproc, events = "TRIALID {trial}")
#> 
#> $params$epoch$parameters
#> $params$epoch$parameters$events
#> [1] "TRIALID {trial}"
...
#> 
#> $confounds$epoched_timeseries
#> $confounds$epoched_timeseries$epoch_trialidTrial
#> $confounds$epoched_timeseries$epoch_trialidTrial$block_1
#>                                                               matched_event
#> TRIALID 22.pupil_raw                                             TRIALID 22
#> TRIALID 22.pupil_raw_deblink                                     TRIALID 22
#> TRIALID 22.pupil_raw_deblink_detransient                         TRIALID 22
#> TRIALID 22.pupil_raw_deblink_detransient_interpolate             TRIALID 22
#> TRIALID 22.pupil_raw_deblink_detransient_interpolate_lpfilt      TRIALID 22
#> TRIALID 22.pupil_raw_deblink_detransient_interpolate_lpfilt_z    TRIALID 22
#> TRIALID 23.pupil_raw                                             TRIALID 23
#> TRIALID 23.pupil_raw_deblink                                     TRIALID 23
#> TRIALID 23.pupil_raw_deblink_detransient                         TRIALID 23
#> TRIALID 23.pupil_raw_deblink_detransient_interpolate             TRIALID 23
#> TRIALID 23.pupil_raw_deblink_detransient_interpolate_lpfilt      TRIALID 23
#> TRIALID 23.pupil_raw_deblink_detransient_interpolate_lpfilt_z    TRIALID 23
#> TRIALID 24.pupil_raw                                             TRIALID 24
#> TRIALID 24.pupil_raw_deblink                                     TRIALID 24
#> TRIALID 24.pupil_raw_deblink_detransient                         TRIALID 24
#> TRIALID 24.pupil_raw_deblink_detransient_interpolate             TRIALID 24
#> TRIALID 24.pupil_raw_deblink_detransient_interpolate_lpfilt      TRIALID 24
#> TRIALID 24.pupil_raw_deblink_detransient_interpolate_lpfilt_z    TRIALID 24
#> TRIALID 25.pupil_raw                                             TRIALID 25
#> TRIALID 25.pupil_raw_deblink                                     TRIALID 25
#> TRIALID 25.pupil_raw_deblink_detransient                         TRIALID 25
#> TRIALID 25.pupil_raw_deblink_detransient_interpolate             TRIALID 25
#> TRIALID 25.pupil_raw_deblink_detransient_interpolate_lpfilt      TRIALID 25
#> TRIALID 25.pupil_raw_deblink_detransient_interpolate_lpfilt_z    TRIALID 25
#> TRIALID 26.pupil_raw                                             TRIALID 26
#> TRIALID 26.pupil_raw_deblink                                     TRIALID 26
#> TRIALID 26.pupil_raw_deblink_detransient                         TRIALID 26
#> TRIALID 26.pupil_raw_deblink_detransient_interpolate             TRIALID 26
#> TRIALID 26.pupil_raw_deblink_detransient_interpolate_lpfilt      TRIALID 26
#> TRIALID 26.pupil_raw_deblink_detransient_interpolate_lpfilt_z    TRIALID 26
#>                                                               text_unique
#> TRIALID 22.pupil_raw                                           TRIALID 22
#> TRIALID 22.pupil_raw_deblink                                   TRIALID 22
#> TRIALID 22.pupil_raw_deblink_detransient                       TRIALID 22
...
#> TRIALID 26.pupil_raw_deblink_detransient_interpolate_lpfilt_z  TRIALID 26
#>                                                                                                       step
#> TRIALID 22.pupil_raw                                                                                   raw
#> TRIALID 22.pupil_raw_deblink                                                                   raw_deblink
#> TRIALID 22.pupil_raw_deblink_detransient                                           raw_deblink_detransient
#> TRIALID 22.pupil_raw_deblink_detransient_interpolate                   raw_deblink_detransient_interpolate
#> TRIALID 22.pupil_raw_deblink_detransient_interpolate_lpfilt     raw_deblink_detransient_interpolate_lpfilt
#> TRIALID 22.pupil_raw_deblink_detransient_interpolate_lpfilt_z raw_deblink_detransient_interpolate_lpfilt_z
#> TRIALID 23.pupil_raw                                                                                   raw
#> TRIALID 23.pupil_raw_deblink                                                                   raw_deblink
#> TRIALID 23.pupil_raw_deblink_detransient                                           raw_deblink_detransient
#> TRIALID 23.pupil_raw_deblink_detransient_interpolate                   raw_deblink_detransient_interpolate
#> TRIALID 23.pupil_raw_deblink_detransient_interpolate_lpfilt     raw_deblink_detransient_interpolate_lpfilt
#> TRIALID 23.pupil_raw_deblink_detransient_interpolate_lpfilt_z raw_deblink_detransient_interpolate_lpfilt_z
#> TRIALID 24.pupil_raw                                                                                   raw
#> TRIALID 24.pupil_raw_deblink                                                                   raw_deblink
#> TRIALID 24.pupil_raw_deblink_detransient                                           raw_deblink_detransient
#> TRIALID 24.pupil_raw_deblink_detransient_interpolate                   raw_deblink_detransient_interpolate
#> TRIALID 24.pupil_raw_deblink_detransient_interpolate_lpfilt     raw_deblink_detransient_interpolate_lpfilt
#> TRIALID 24.pupil_raw_deblink_detransient_interpolate_lpfilt_z raw_deblink_detransient_interpolate_lpfilt_z
#> TRIALID 25.pupil_raw                                                                                   raw
#> TRIALID 25.pupil_raw_deblink                                                                   raw_deblink
#> TRIALID 25.pupil_raw_deblink_detransient                                           raw_deblink_detransient
#> TRIALID 25.pupil_raw_deblink_detransient_interpolate                   raw_deblink_detransient_interpolate
#> TRIALID 25.pupil_raw_deblink_detransient_interpolate_lpfilt     raw_deblink_detransient_interpolate_lpfilt
#> TRIALID 25.pupil_raw_deblink_detransient_interpolate_lpfilt_z raw_deblink_detransient_interpolate_lpfilt_z
#> TRIALID 26.pupil_raw                                                                                   raw
#> TRIALID 26.pupil_raw_deblink                                                                   raw_deblink
#> TRIALID 26.pupil_raw_deblink_detransient                                           raw_deblink_detransient
...
#> TRIALID 22.pupil_raw_deblink_detransient                      1051.000000
#> TRIALID 22.pupil_raw_deblink_detransient_interpolate          1051.000000
#> TRIALID 22.pupil_raw_deblink_detransient_interpolate_lpfilt   1010.411304
#> TRIALID 22.pupil_raw_deblink_detransient_interpolate_lpfilt_z    2.226711
#> TRIALID 23.pupil_raw                                          1699.000000
#> TRIALID 23.pupil_raw_deblink                                  1699.000000
#> TRIALID 23.pupil_raw_deblink_detransient                      1699.000000
#> TRIALID 23.pupil_raw_deblink_detransient_interpolate          1699.000000
#> TRIALID 23.pupil_raw_deblink_detransient_interpolate_lpfilt   1660.799078
#> TRIALID 23.pupil_raw_deblink_detransient_interpolate_lpfilt_z    3.660014
#> TRIALID 24.pupil_raw                                          1129.000000
#> TRIALID 24.pupil_raw_deblink                                  1129.000000
#> TRIALID 24.pupil_raw_deblink_detransient                      1129.000000
#> TRIALID 24.pupil_raw_deblink_detransient_interpolate          1129.000000
#> TRIALID 24.pupil_raw_deblink_detransient_interpolate_lpfilt   1080.197345
#> TRIALID 24.pupil_raw_deblink_detransient_interpolate_lpfilt_z    2.380503
#> TRIALID 25.pupil_raw                                          2623.000000
#> TRIALID 25.pupil_raw_deblink                                   801.000000
#> TRIALID 25.pupil_raw_deblink_detransient                       801.000000
#> TRIALID 25.pupil_raw_deblink_detransient_interpolate           801.000000
#> TRIALID 25.pupil_raw_deblink_detransient_interpolate_lpfilt    762.593460
#> TRIALID 25.pupil_raw_deblink_detransient_interpolate_lpfilt_z    1.680578
#> TRIALID 26.pupil_raw                                          1233.000000
#> TRIALID 26.pupil_raw_deblink                                  1233.000000
#> TRIALID 26.pupil_raw_deblink_detransient                      1233.000000
#> TRIALID 26.pupil_raw_deblink_detransient_interpol
#> $epoch_trialidTrial$info
#> $epoch_trialidTrial$info$block_1
#> $epoch_trialidTrial$info$block_1$calc_baseline
#> [1] FALSE
#> 
#> $epoch_trialidTrial$info$block_1$apply_baseline
#> [1] FALSE
#> 
#> $epoch_trialidTrial$info$block_1$epoch_events
#> [1] "TRIALID {trial}"
#> 
#> $epoch_trialidTrial$info$block_1$epoch_limits
#> NULL
#> 
#> $epoch_trialidTrial$info$block_1$n_epochs
#> [1] 5
#> 
#> 
#> 
...
#> ℹ [2026-07-03 18:41:00] [INFO] Epoching pupil data...
#> ℹ [2026-07-03 18:41:00] [INFO] Block 1: found 5 matching events for
#> PROBESTARTtrial
#> ✔ [2026-07-03 18:41:00] [OKAY] Done!
#> ✔ [2026-07-03 18:41:00] [OKAY] Block 1: pupil data from 5 unique event messages
#> extracted
#> ✔ [2026-07-03 18:41:00] [OKAY] Pupil epoching completed in 0.07 seconds
#> ℹ [2026-07-03 18:41:00] [INFO] Recalculating epoched confounds for new
#> epochs...
#> $file
#> [1] "/home/runner/work/_temp/Library/eyeris/extdata/memory.asc"
#> 
...
#> $params$epoch
#> $params$epoch$call_stack
#> eyeris::epoch(eyeris = eye_preproc, events = "PROBE_START_{trial}", 
#>     limits = c(0, 1))
#> 
#> $params$epoch$parameters
#> $params$epoch$parameters$events
...
#> 
#> $confounds$epoched_timeseries
#> $confounds$epoched_timeseries$epoch_probeStartTrial
#> $confounds$epoched_timeseries$epoch_probeStartTrial$block_1
#>                                                                    matched_event
#> PROBE_START_22.pupil_raw                                          PROBE_START_22
#> PROBE_START_22.pupil_raw_deblink                                  PROBE_START_22
#> PROBE_START_22.pupil_raw_deblink_detransient                      PROBE_START_22
...
#> PROBE_START_22.pupil_raw_deblink_detransient                       998.0000000
#> PROBE_START_22.pupil_raw_deblink_detransient_interpolate           998.0000000
#> PROBE_START_22.pupil_raw_deblink_detransient_interpolate_lpfilt    971.1301965
#> PROBE_START_22.pupil_raw_deblink_detransient_interpolate_lpfilt_z    2.1401446
#> PROBE_START_23.pupil_raw                                          1260.0000000
#> PROBE_START_23.pupil_raw_deblink                                  1260.0000000
#> PROBE_START_23.pupil_raw_deblink_detransient                      1260.0000000
#> PROBE_START_23.pupil_raw_deblink_detransient_interpolate          1260.0000000
#> PROBE_START_23.pupil_raw_deblink_detransient_interpolate_lpfilt   1231.7028625
#> PROBE_START_23.pupil_raw_deblink_detransient_interpolate_lpfilt_z    2.7143860
#> PROBE_START_24.pupil_raw                                           889.0000000
#> PROBE_START_24.pupil_raw_deblink                                   889.0000000
#> PROBE_START_24.pupil_raw_deblink_detransient                       889.0000000
#> PROBE_START_24.pupil_raw_deblink_detransient_interpolate           889.0000000
#> PROBE_START_24.pupil_raw_deblink_detransient_interpolate_lpfilt    861.9328918
#> PROBE_START_24.pupil_raw_deblink_detransient_interpolate_lpfilt_z    1.8994992
#> PROBE_START_25.pupil_raw                                           320.0000000
#> PROBE_START_25.pupil_raw_deblink                                   320.0000000
#> PROBE_START_25.pupil_raw_deblink_detransient                       320.0000000
#> PROBE_START_25.pupil_raw_deblink_detransient_interpolate           320.0000000
#> PROBE_START_25.pupil_raw_deblink_detransient_interpolate_lpfilt    258.3466336
#> PROBE_START_25.pupil_raw_deblink_detransient_interpolate_lpfilt_z    0.5693358
#> PROBE_START_26.pupil_raw                                          1046.0000000
#> PROBE_START_26.pupil_raw_deblink                                  1046.0000000
#> PROBE_START_26.pupil_raw_deblink_detransient                      1046.0000000
#> PROBE_START_26.pupil_raw_deblink_detransient_interpolate          1046.0000000
#> PROBE_START_26.pupil_raw_deblink_detransient_interpolate_lpfilt    991.3022946
#> PROBE_START_26.pupil_raw_deblink_detransient_interpolate_lpfilt_z    2.1845992
#>                                                                   zscore_max
#> PROBE_START_22.pupil_raw                                            1.212828
#> PROBE_START_22.pupil_raw_deblink                                    1.212828
#> PROBE_START_22.pupil_raw_deblink_detransient                        1.212828
#> PROBE_START_22.pupil_raw_deblink_detransient_interpolate            1.212828
#> PROBE_START_22.pupil_raw_deblink_detransient_interpolate_lpfilt     1.171161
#> PROBE_START_22.pupil_raw_deblink_detransient_interpolate_lpfilt_z   1.171161
#> PROBE_START_23.pupil_raw                                            1.187923
#> PROBE_START_23.pupil_raw_deblink                                    1.187923
#> PROBE_START_23.pupil_raw_deblink_detransient                        1.187923
#> PROBE_START_23.pupil_raw_deblink_detransient_interpolate            1.187923
#> PROBE_START_23.pupil_raw_deblink_detransient_interpolate_lpfilt     1.143733
#> PROBE_START_23.pupil_raw_deblink_detransient_interpolate_lpfilt_z   1.143733
#> PROBE_START_24.pupil_raw                                            1.426172
#> PROBE_START_24.pupil_raw_deblink                                    1.426172
#> PROBE_START_24.pupil_raw_deblink_detransient                        1.426172
#> PROBE_START_24.pupil_raw_deblink_detransient_interpolate            1.426172
#> PROBE_START_24.pupil_raw_deblink_detransient_interpolate_lpfilt     1.371809
#> PROBE_START_24.pupil_raw_deblink_detransient_interpolate_lpfilt_z   1.371809
#> PROBE_START_25.pupil_raw                                            1.319130
#> PROBE_START_25.pupil_raw_deblink                                    1.319130
#> PROBE_START_25.pupil_raw_deblink_detransient                        1.319130
#> PROBE_START_25.pupil_raw_deblink_detransient_interpolate            1.319130
#> PROBE_START_25.pupil_raw_deblink_detransient_interpolate_lpfilt     1.053828
#> PROBE_START_25.pupil_raw_deblink_detransient_interpolate_lpfilt_z   1.053828
#> PROBE_START_26.pupil_raw                                            1.101652
#> PROBE_START_26.pupil_raw_deblink                                    1.101652
#> PROBE_START_26.pupil_raw_deblink_detrans
#> PROBE_START_26.pupil_raw_deblink_detransient_interpolate_lpfilt_z      0.7871904
#> 
#> 
#> 
#> $confounds$epoched_epoch_wide
#> $confounds$epoched_epoch_wide$epoch_probeStartTrial
#> $confounds$epoched_epoch_wide$epoch_probeStartTrial$block_1
#>                 matched_event    text_unique n_samples n_blinks_in_baseline
#> PROBE_START_22 PROBE_START_22 PROBE_START_22      1000                   NA
#> PROBE_START_23 PROBE_START_23 PROBE_START_23      1000                   NA
#> PROBE_START_24 PROBE_START_24 PROBE_START_24      1000                   NA
#> PROBE_START_25 PROBE_START_25 PROBE_START_25      1000                   NA
#> PROBE_START_26 PROBE_START_26 PROBE_START_26      1000                   NA
#>                time_to_first_blink_ms epoch_duration_ms
#> PROBE_START_22                     NA               999
#> PROBE_START_23                     NA               999
#> PROBE_START_24                     NA               999
#> PROBE_START_25                     NA               999
#> PROBE_START_26                     NA               999
#> 
#> 
#> 
#> 
...
#> #   text_unique <chr>, template <chr>, matching_pattern <chr>, …
#> 
#> $epoch_probeStartTrial$info
#> $epoch_probeStartTrial$info$block_1
#> $epoch_probeStartTrial$info$block_1$calc_baseline
#> [1] FALSE
#> 
#> $epoch_probeStartTrial$info$block_1$apply_baseline
#> [1] FALSE
#> 
#> $epoch_probeStartTrial$info$block_1$epoch_events
#> [1] "PROBE_START_{trial}"
#> 
#> $epoch_probeStartTrial$info$block_1$epoch_limits
#> [1] 0 1
#> 
#> $epoch_probeStartTrial$info$block_1$n_epochs
#> [1] 5
#> 
#> 
#> 
...
#> $params$epoch$call_stack
#> eyeris::epoch(eyeris = eye_preproc, events = "PROBE_START_{trial}", 
#>     limits = c(-1, 1), label = "prePostProbe")
#> 
#> $params$epoch$parameters
#> $params$epoch$parameters$events
#> [1] "PROBE_START_{trial}"
...
#> 
#> $confounds$epoched_timeseries
#> $confounds$epoched_timeseries$epoch_prePostProbe
#> $confounds$epoched_timeseries$epoch_prePostProbe$block_1
#>                                                                    matched_event
#> PROBE_START_22.pupil_raw                                          PROBE_START_22
#> PROBE_START_22.pupil_raw_deblink                                  PROBE_START_22
#> PROBE_START_22.pupil_raw_deblink_detransient                      PROBE_START_22
...
#> 
#> $confounds$epoched_epoch_wide
#> $confounds$epoched_epoch_wide$epoch_prePostProbe
#> $confounds$epoched_epoch_wide$epoch_prePostProbe$block_1
#>                 matched_event    text_unique n_samples n_blinks_in_baseline
#> PROBE_START_22 PROBE_START_22 PROBE_START_22      2000                   NA
#> PROBE_START_23 PROBE_START_23 PROBE_START_23      2000                   NA
#> PROBE_START_24 PROBE_START_24 PROBE_START_24      2000                   NA
...
#> #   text_unique <chr>, template <chr>, matching_pattern <chr>, …
#> 
#> $epoch_prePostProbe$info
#> $epoch_prePostProbe$info$block_1
#> $epoch_prePostProbe$info$block_1$calc_baseline
#> [1] FALSE
#> 
#> $epoch_prePostProbe$info$block_1$apply_baseline
#> [1] FALSE
#> 
#> $epoch_prePostProbe$info$block_1$epoch_events
#> [1] "PROBE_START_{trial}"
#> 
#> $epoch_prePostProbe$info$block_1$epoch_limits
#> [1] -1  1
#> 
#> $epoch_prePostProbe$info$block_1$n_epochs
#> [1] 5
#> 
#> 
#> 
...
#> ℹ [2026-07-03 18:41:00] [INFO] Epoching pupil data...
#> ! [2026-07-03 18:41:00] [WARN] Manual epoching only works with 1 block at a
...
#> ✔ [2026-07-03 18:41:00] [OKAY] Done!
#> ✔ [2026-07-03 18:41:00] [OKAY] Block 1: pupil data from 1 unique event messages
#> extracted
#> ✔ [2026-07-03 18:41:00] [OKAY] Pupil epoching completed in 0.03 seconds
#> ℹ [2026-07-03 18:41:00] [INFO] Recalculating epoched confounds for new
#> epochs...
#> $file
#> [1] "/home/runner/work/_temp/Library/eyeris/extdata/memory.asc"
...
#> $params$epoch
#> $params$epoch$call_stack
#> eyeris::epoch(eyeris = eye_preproc, events = list(data.frame(time = c(11334491), 
#>     msg = c("TRIALID 22")), data.frame(time = c(11337158), msg = c("RESPONSE_22")), 
#>     1), label = "example5")
#> 
#> $params$epoch$parameters
#> $params$epoch$parameters$events
#> $params$epoch$parameters$events[[1]]
#>       time        msg
#> 1 11334491 TRIALID 22
#> 
#> $params$epoch$parameters$events[[2]]
#>       time         msg
#> 1 11337158 RESPONSE_22
#> 
#> $params$epoch$parameters$events[[3]]
#> [1] 1
#> 
#> 
#> $params$epoch$parameters$limits
...
#> 
#> $confounds$epoched_timeseries
#> $confounds$epoched_timeseries$epoch_example5
#> list()
#> 
#> 
#> $confounds$epoched_epoch_wide
#> $confounds$epoched_epoch_wide$epoch_example5
#> list()
#> 
#> 
#> 
...
#> 10     1  11334500     0.009       0.009  983.  552. R      1000 diam…      6199
#> # ℹ 2,657 more rows
#> # ℹ 10 more variables: pupil_raw_deblink <dbl>,
#> #   pupil_raw_deblink_detransient <dbl>,
#> #   pupil_raw_deblink_detransient_interpolate <dbl>,
#> #   pupil_raw_deblink_detransient_interpolate_lpfilt <dbl>,
#> #   pupil_raw_deblink_detransient_interpolate_lpfilt_z <dbl>, timebin <dbl>,
...
#> 
#> $epoch_example5$info
#> $epoch_example5$info$block_1
#> $epoch_example5$info$block_1$calc_baseline
#> [1] FALSE
#> 
#> $epoch_example5$info$block_1$apply_baseline
#> [1] FALSE
#> 
#> $epoch_example5$info$block_1$epoch_events
#> $epoch_example5$info$block_1$epoch_events[[1]]
#>       time        msg
#> 1 11334491 TRIALID 22
#> 
#> $epoch_example5$info$block_1$epoch_events[[2]]
#>       time         msg
#> 1 11337158 RESPONSE_22
#> 
#> $epoch_example5$info$block_1$epoch_events[[3]]
#> [1] 1
#> 
#> 
#> $epoch_example5$info$block_1$epoch_limits
#> NULL
#> 
#> $epoch_example5$info$block_1$n_epochs
#> [1] 1
#> 
#> 
#> 
...
#> $params$epoch$call_stack
#> eyeris::epoch(eyeris = eye_preproc, events = list(data.frame(time = c(11334491), 
#>     msg = NA), data.frame(time = c(11337158), msg = NA), 1), 
#>     label = "example6")
#> 
#> $params$epoch$parameters
#> $params$epoch$parameters$events
...
#> 
#> $confounds$epoched_timeseries
#> $confounds$epoched_timeseries$epoch_example6
#> list()
#> 
#> 
#> $confounds$epoched_epoch_wide
#> $confounds$epoched_epoch_wide$epoch_example6
#> list()
#> 
#> 
#> 
...
#> 
#> $epoch_example6$info
#> $epoch_example6$info$block_1
#> $epoch_example6$info$block_1$calc_baseline
#> [1] FALSE
#> 
#> $epoch_example6$info$block_1$apply_baseline
#> [1] FALSE
#> 
#> $epoch_example6$info$block_1$epoch_events
#> $epoch_example6$info$block_1$epoch_events[[1]]
#>       time msg
#> 1 11334491  NA
#> 
#> $epoch_example6$info$block_1$epoch_events[[2]]
#>       time msg
#> 1 11337158  NA
#> 
#> $epoch_example6$info$block_1$epoch_events[[3]]
#> [1] 1
#> 
#> 
#> $epoch_example6$info$block_1$epoch_limits
#> NULL
#> 
#> $epoch_example6$info$block_1$n_epochs
#> [1] 1
#> 
#> 
#> 
...
#> ℹ [2026-07-03 18:41:01] [INFO] Epoching and baselining pupil data...
#> ℹ [2026-07-03 18:41:01] [INFO] Block 1: found 5 matching events for
#> PROBESTARTtrial
#> ✔ [2026-07-03 18:41:01] [OKAY] Done!
#> ✔ [2026-07-03 18:41:01] [OKAY] Block 1: pupil data from 5 unique event messages
#> extracted
#> ✔ [2026-07-03 18:41:01] [OKAY] Block 1: 9 epochs baselined
#> ✔ [2026-07-03 18:41:01] [OKAY] Pupil epoching and baselining completed in 0.09
#> secs
#> ℹ [2026-07-03 18:41:01] [INFO] Recalculating epoched confounds for new
#> epochs...
#> $file
#> [1] "/home/runner/work/_temp/Library/eyeris/extdata/memory.asc"
#> 
...
#> $params$epoch$call_stack
#> eyeris::epoch(eyeris = eye_preproc, events = "PROBE_START_{trial}", 
#>     limits = c(0, 1), label = "prePostProbe", baseline = TRUE, 
#>     baseline_type = "sub", baseline_events = "DELAY_STOP_*", 
#>     baseline_period = c(-1, 0))
#> 
#> $params$epoch$parameters
#> $params$epoch$parameters$events
...
#> PROBE_START_22.pupil_raw_deblink_detransient_interpolate_lpfilt                         PROBE_START_22
#> PROBE_START_22.pupil_raw_deblink_detransient_interpolate_lpfilt_z                       PROBE_START_22
#> PROBE_START_22.pupil_raw_deblink_detransient_interpolate_lpfilt_sub_bline_DELAY_STOP_   PROBE_START_22
#> PROBE_START_22.pupil_raw_deblink_detransient_interpolate_lpfilt_sub_bline_DELAY_STOP__z PROBE_START_22
#> PROBE_START_23.pupil_raw                                                                PROBE_START_23
#> PROBE_START_23.pupil_raw_deblink                                                        PROBE_START_23
#> PROBE_START_23.pupil_raw_deblink_detransient                                            PROBE_START_23
#> PROBE_START_23.pupil_raw_deblink_detransient_interpolate                                PROBE_START_23
#> PROBE_START_23.pupil_raw_deblink_detransient_interpolate_lpfilt                         PROBE_START_23
#> PROBE_START_23.pupil_raw_deblink_detransient_interpolate_lpfilt_z                       PROBE_START_23
#> PROBE_START_23.pupil_raw_deblink_detransient_interpolate_lpfilt_sub_bline_DELAY_STOP_   PROBE_START_23
#> PROBE_START_23.pupil_raw_deblink_detransient_interpolate_lpfilt_sub_bline_DELAY_STOP__z PROBE_START_23
#> PROBE_START_24.pupil_raw                                                                PROBE_START_24
#> PROBE_START_24.pupil_raw_deblink                                                        PROBE_START_24
#> PROBE_START_24.pupil_raw_deblink_detransient                                            PROBE_START_24
#> PROBE_START_24.pupil_raw_deblink_detransient_interpolate                                PROBE_START_24
#> PROBE_START_24.pupil_raw_deblink_detransient_interpolate_lpfilt                         PROBE_START_24
#> PROBE_START_24.pupil_raw_deblink_detransient_interpolate_lpfilt_z                       PROBE_START_24
#> PROBE_START_24.pupil_raw_deblink_detransient_interpolate_lpfilt_sub_bline_DELAY_STOP_   PROBE_START_24
#> PROBE_START_24.pupil_raw_deblink_detransient_interpolate_lpfilt_sub_bline_DELAY_STOP__z PROBE_START_24
#> PROBE_START_25.pupil_raw                                                                PROBE_START_25
#> PROBE_START_25.pupil_raw_deblink                                                        PROBE_START_25
#> PROBE_START_25.pupil_raw_deblink_detransient                                            PROBE_START_25
#> PROBE_START_25.pupil_raw_deblink_detransient_interpolate                                PROBE_START_25
#> PROBE_START_25.pupil_raw_deblink_detransient_interpolate_lpfilt                         PROBE_START_25
#> PROBE_START_25.pupil_raw_deblink_detransient_interpolate_lpfilt_z                       PROBE_START_25
#> PROBE_START_25.pupil_raw_deblink_detransient_interpolate_lpfilt_sub_bline_DELAY_STOP_   PROBE_START_25
#> PROBE_START_25.pupil_raw_deblink_detransient_interpolate_lpfilt_sub_bline_DELAY_STOP__z PROBE_START_25
#> PROBE_START_26.pupil_raw                                                                PROBE_START_26
#> PROBE_START_26.pupil_raw_deblink                                                        PROBE_START_26
#> PROBE_START_26.pupil_raw_deblink_detransient                                            PROBE_START_26
#> PROBE_START_26.pupil_raw_deblink_detransient_interpolate                                PROBE_START_26
#> PROBE_START_26.pupil_raw_deblink_detransient_interpolate_lpfilt                         PROBE_START_26
#> PROBE_START_26.pupil_raw_deblink_detransient_interpolate_lpfilt_z                       PROBE_START_26
#> PROBE_START_26.pupil_raw_deblink_detransient_interpolate_lpfilt_sub_bline_DELAY_STOP_   PROBE_START_26
#> PROBE_START_26.pupil_raw_deblink_detransient_interpolate_lpfilt_sub_bline_DELAY_STOP__z PROBE_START_26
#>                                                                                            text_unique
#> PROBE_START_22.pupil_raw                                                                PROBE_START_22
#> PROBE_START_22.pupil_raw_deblink                                                        PROBE_START_22
#> PROBE_START_22.pupil_raw_deblink_detransient                                            PROBE_START_22
...
#> PROBE_START_22.pupil_raw_deblink_detransient_interpolate_lpfilt                                                 raw_deblink_detransient_interpolate_lpfilt
#> PROBE_START_22.pupil_raw_deblink_detransient_interpolate_lpfilt_z                                             raw_deblink_detransient_interpolate_lpfilt_z
#> PROBE_START_22.pupil_raw_deblink_detransient_interpolate_lpfilt_sub_bline_DELAY_STOP_     raw_deblink_detransient_interpolate_lpfilt_sub_bline_DELAY_STOP_
#> PROBE_START_22.pupil_raw_deblink_detransient_interpolate_lpfilt_sub_bline_DELAY_STOP__z raw_deblink_detransient_interpolate_lpfilt_sub_bline_DELAY_STOP__z
#> PROBE_START_23.pupil_raw                                                                                                                               raw
#> PROBE_START_23.pupil_raw_deblink                                                                                                               raw_deblink
#> PROBE_START_23.pupil_raw_deblink_detransient                                                                                       raw_deblink_detransient
#> PROBE_START_23.pupil_raw_deblink_detransient_interpolate                                                               raw_deblink_detransient_interpolate
#> PROBE_START_23.pupil_raw_deblink_detransient_interpolate_lpfilt                                                 raw_deblink_detransient_interpolate_lpfilt
#> PROBE_START_23.pupil_raw_deblink_detransient_interpolate_lpfilt_z                                             raw_deblink_detransient_interpolate_lpfilt_z
#> PROBE_START_23.pupil_raw_deblink_detransient_interpolate_lpfilt_sub_bline_DELAY_STOP_     raw_deblink_detransient_interpolate_lpfilt_sub_bline_DELAY_STOP_
#> PROBE_START_23.pupil_raw_deblink_detransient_interpolate_lpfilt_sub_bline_DELAY_STOP__z raw_deblink_detransient_interpolate_lpfilt_sub_bline_DELAY_STOP__z
#> PROBE_START_24.pupil_raw                                                                                                                               raw
#> PROBE_START_24.pupil_raw_deblink                                                                                                               raw_deblink
#> PROBE_START_24.pupil_raw_deblink_detransient                                                                                       raw_deblink_detransient
#> PROBE_START_24.pupil_raw_deblink_detransient_interpolate                                                               raw_deblink_detransient_interpolate
#> PROBE_START_24.pupil_raw_deblink_detransient_interpolate_lpfilt                                                 raw_deblink_detransient_interpolate_lpfilt
#> PROBE_START_24.pupil_raw_deblink_detransient_interpolate_lpfilt_z                                             raw_deblink_detransient_interpolate_lpfilt_z
#> PROBE_START_24.pupil_raw_deblink_detransient_interpolate_lpfilt_sub_bline_DELAY_STOP_     raw_deblink_detransient_interpolate_lpfilt_sub_bline_DELAY_STOP_
#> PROBE_START_24.pupil_raw_deblink_detransient_interpolate_lpfilt_sub_bline_DELAY_STOP__z raw_deblink_detransient_interpolate_lpfilt_sub_bline_DELAY_STOP__z
#> PROBE_START_25.pupil_raw                                                                                                                               raw
#> PROBE_START_25.pupil_raw_deblink                                                                                                               raw_deblink
#> PROBE_START_25.pupil_raw_deblink_detransient                                                                                       raw_deblink_detransient
#> PROBE_START_25.pupil_raw_deblink_detransient_interpolate                
1.713636
#> PROBE_START_25.pupil_raw_deblink_detransient                                             -1.713636
#> PROBE_START_25.pupil_raw_deblink_detransient_interpolate                                 -1.713636
#> PROBE_START_25.pupil_raw_deblink_detransient_interpolate_lpfilt                          -1.405919
#> PROBE_START_25.pupil_raw_deblink_detransient_interpolate_lpfilt_z                        -1.405919
#> PROBE_START_25.pupil_raw_deblink_detransient_interpolate_lpfilt_sub_bline_DELAY_STOP_    -1.405919
#> PROBE_START_25.pupil_raw_deblink_detransient_interpolate_lpfilt_sub_bline_DELAY_STOP__z  -1.405919
#> PROBE_START_26.pupil_raw                                                                 -1.825580
#> PROBE_START_26.pupil_raw_deblink                                                         -1.825580
#> PROBE_START_26.pupil_raw_deblink_detransient                                             -1.825580
#> PROBE_START_26.pupil_raw_deblink_detransient_interpolate                                 -1.825580
#> PROBE_START_26.pupil_raw_deblink_detransient_interpolate_lpfilt                          -1.737069
#> PROBE_START_26.pupil_raw_deblink_detransient_interpolate_lpfilt_z                        -1.737069
#> PROBE_START_26.pupil_raw_deblink_detransient_interpolate_lpfilt_sub_bline_DELAY_STOP_    -1.737069
#> PROBE_START_26.pupil_raw_deblink_detransient_interpolate_lpfilt_sub_bline_DELAY_STOP__z  -1.737069
#>                                                                                         prop_blink_time
#> PROBE_START_22.pupil_raw                                                                              0
#> PROBE_START_22.pupil_raw_deblink                                                                      0
#> PROBE_START_22.pupil_raw_deblink_detransient                                                          0
...
#> PROBE_START_22.pupil_raw_deblink_detransient_interpolate_lpfilt                                42.09339080
#> PROBE_START_22.pupil_raw_deblink_detransient_interpolate_lpfilt_z                               0.09276402
#> PROBE_START_22.pupil_raw_deblink_detransient_interpolate_lpfilt_sub_bline_DELAY_STOP_                   NA
#> PROBE_START_22.pupil_raw_deblink_detransient_interpolate_lpfilt_sub_bline_DELAY_STOP__z                 NA
#> PROBE_START_23.pupil_raw                                                                       41.04019932
#> PROBE_START_23.pupil_raw_deblink                                                               41.04019932
#> PROBE_START_23.pupil_raw_deblink_detransient                                                   41.04019932
#> PROBE_START_23.pupil_raw_deblink_detransient_interpolate                                       41.04019932
#> PROBE_START_23.pupil_raw_deblink_detransient_interpolate_lpfilt                                39.02897700
#> PROBE_START_23.pupil_raw_deblink_detransient_interpolate_lpfilt_z                               0.08601077
#> PROBE_START_23.pupil_raw_deblink_detransient_interpolate_lpfilt_sub_bline_DELAY_STOP_                   NA
#> PROBE_START_23.pupil_raw_deblink_detransient_interpolate_lpfilt_sub_bline_DELAY_STOP__z                 NA
#> PROBE_START_24.pupil_raw                                                                       62.93071686
#> PROBE_START_24.pupil_raw_deblink                                                               62.93071686
#> PROBE_START_24.pupil_raw_deblink_detransient                                                   62.93071686
#> PROBE_START_24.pupil_raw_deblink_detransient_interpolate                                       62.93071686
#> PROBE_START_24.pupil_raw_deblink_detransient_interpolate_lpfilt                                61.28090023
#> PROBE_START_24.pupil_raw_deblink_detransient_interpolate_lpfilt_z                               0.13504882
#> PROBE_START_24.pupil_raw_deblink_detransient_interpolate_lpfilt_sub_bline_DELAY_STOP_                   NA
#> PROBE_START_24.pupil_raw_deblink_detransient_interpolate_lpfilt_sub_bline_DELAY_STOP__z                 NA
#> PROBE_START_25.pupil_raw                                                                       40.62699823
#> PROBE_START_25.pupil_raw_deblink                                                               40.62699823
#> PROBE_START_25.pupil_raw_deblink_detransient                                                   40.62699823
#> PROBE_START_25.pupil_raw_deblink_detransient_interpolate                                       40.62699823
#> PROBE_START_25.pupil_raw_deblink_detransient_interpolate_lpfilt                                40.73405825
#> PROBE_START_25.pupil_raw_deblink_detransient_interpolate_lpfilt_z                               0.08976837
#> PROBE_START_25.pupil_raw_deblink_detransient_interpolate_lpfilt_sub_bline_DELAY_STOP_                   NA
#> PROBE_START_25.pupil_raw_deblink_detransient_interpolate_lpfilt_sub_bline_DELAY_STOP__z                 NA
#> PROBE_START_26.pupil_raw                                                                       20.27918326
#> PROBE_START_26.pupil_raw_deblink                                                               20.27918326
#> PROBE_START_26.pupil_raw_deblink_detransient                                                   20.27918326
#> PROBE_START_26.pupil_raw_deblink_detransient_interpolate                                       20.27918326
#> PROBE_START_26.pupil_raw_deblink_detransient_interpolate_lpfilt                                20.61066743
#> PROBE_START_26.pupil_raw_deblink_detransient_interpolate_lpfilt_z                               0.04542111
#> PROBE_START_26.pupil_raw_deblink_detransient_interpolate_lpfilt_sub_bline_DELAY_STOP_                   NA
#> PROBE_START_26.pupil_raw_deblink_detransient_interpolate_lpfilt_sub_bline_DELAY_STOP__z                 NA
#>                                                                                         epoch_pupil_sd
#> PROBE_START_22.pupil_raw                                                                   326.8312387
#> PROBE_START_22.pupil_raw_deblink                                                           326.8312387
#> PROBE_START_22.pupil_raw_deblink_detransient                                               326.8312387
...
#> PROBE_START_22.pupil_raw_deblink_detransient_interpolate_lpfilt                            326.5297980
#> PROBE_START_22.pupil_raw_deblink_detransient_interpolate_lpfilt_z                            0.7195956
#> PROBE_START_22.pupil_raw_deblink_detransient_interpolate_lpfilt_sub_bline_DELAY_STOP_      326.5297980
#> PROBE_START_22.pupil_raw_deblink_detransient_interpolate_lpfilt_sub_bline_DELAY_STOP__z      0.7295999
#> PROBE_START_23.pupil_raw                                                                   408.4668023
#> PROBE_START_23.pupil_raw_deblink                                                           408.4668023
#> PROBE_START_23.pupil_raw_deblink_detransient                                               408.4668023
#> PROBE_START_23.pupil_raw_deblink_detransient_interpolate                                   408.4668023
#> PROBE_START_23.pupil_raw_deblink_detransient_interpolate_lpfilt                            408.3975024
#> PROBE_START_23.pupil_raw_deblink_detransient_interpolate_lpfilt_z                            0.9000129
#> PROBE_START_23.pupil_raw_deblink_detransient_interpolate_lpfilt_sub_bline_DELAY_STOP_      408.3975024
#> PROBE_START_23.pupil_raw_deblink_detransient_interpolate_lpfilt_sub_bline_DELAY_STOP__z      0.9125255
#> PROBE_START_24.pupil_raw                                                                   277.3137048
#> PROBE_START_24.pupil_raw_deblink                                                           277.3137048
#> PROBE_START_24.pupil_raw_deblink_detransient                                               277.3137048
#> PROBE_START_24.pupil_raw_deblink_detransient_interpolate                                   277.3137048
#> PROBE_START_24.pupil_raw_deblink_detransient_interpolate_lpfilt                            277.2579621
#> PROBE_START_24.pupil_raw_deblink_detransient_interpolate_lpfilt_z                            0.6110119
#> PROBE_START_24.pupil_raw_deblink_detransient_interpolate_lpfilt_sub_bline_DELAY_STOP_      277.2579621
#> PROBE_START_24.pupil_raw_deblink_detransient_interpolate_lpfilt_sub_bline_DELAY_STOP__z      0.6195067
#> PROBE_START_25.pupil_raw                                                                   105.5142231
#> PROBE_START_25.pupil_raw_deblink                                                           105.5142231
#> PROBE_START_25.pupil_raw_deblink_detransient                                               105.5142231
#> PROBE_START_25.pupil_raw_deblink_detransient_interpolate                                   105.5142231
#> PROBE_START_25.pupil_raw_deblink_detransient_interpolate_lpfilt                            105.0297599
#> PROBE_START_25.pupil_raw_deblink_detransient_interpolate_lpfilt_z                            0.2314611
#> PROBE_START_25.pupil_raw_deblink_detransient_interpolate_lpfilt_sub_bline_DELAY_STOP_      105.0297599
#> PROBE_START_25.pupil_raw_deblink_detransient_interpolate_lpfilt_sub_bline_DELAY_STOP__z      0.2346791
#> PROBE_START_26.pupil_raw                                                                   357.3341023
#> PROBE_START_26.pupil_raw_deblink                                                           357.3341023
#> PROBE_START_26.pupil_raw_deblink_detransient                                               357.3341023
#> PROBE_START_26.pupil_raw_deblink_detransient_interpolate                                   357.3341023
#> PROBE_START_26.pupil_raw_deblink_detransient_interpolate_lpfilt                            357.2021991
#> PROBE_START_26.pupil_raw_deblink_detransient_interpolate_lpfilt_z                            0.7871904
#> PROBE_START_26.pupil_raw_deblink_detransient_interpolate_lpfilt_sub_bline_DELAY_STOP_      357.2021991
#> PROBE_START_26.pupil_raw_deblink_detransient_interpolate_lpfilt_sub_bline_DELAY_STOP__z      0.7981345
#> 
#> 
#> 
#> $confounds$epoched_epoch_wide
...
#> 10     1  11336483 0.00901      1.99        1.99  972.  550. R      1000 diamet…
#> # ℹ 4,990 more rows
#> # ℹ 13 more variables: pupil_raw <dbl>, pupil_raw_deblink <dbl>,
#> #   pupil_raw_deblink_detransient <dbl>,
#> #   pupil_raw_deblink_detransient_interpolate <dbl>,
#> #   pupil_raw_deblink_detransient_interpolate_lpfilt <dbl>,
#> #   pupil_raw_deblink_detransient_interpolate_lpfilt_z <dbl>,
...
#> 
#> 
#> $baseline_pupil_raw_deblink_detransient_interpolate_lpfilt_sub_bline_DELAY_STOP__epoch_prePostProbe
#> $baseline_pupil_raw_deblink_detransient_interpolate_lpfilt_sub_bline_DELAY_STOP__epoch_prePostProbe$block_1
#> $baseline_pupil_raw_deblink_detransient_interpolate_lpfilt_sub_bline_DELAY_STOP__epoch_prePostProbe$block_1$baseline_cor_epochs
#> $baseline_pupil_raw_deblink_detransient_interpolate_lpfilt_sub_bline_DELAY_STOP__epoch_prePostProbe$block_1$baseline_cor_epochs[[1]]
...
#> 
#> $baseline_pupil_raw_deblink_detransient_interpolate_lpfilt_sub_bline_DELAY_STOP__epoch_prePostProbe$block_1$baseline_cor_epochs[[2]]
...
#> 
#> $baseline_pupil_raw_deblink_detransient_interpolate_lpfilt_sub_bline_DELAY_STOP__epoch_prePostProbe$block_1$baseline_cor_epochs[[3]]
...
#> 
#> $baseline_pupil_raw_deblink_detransient_interpolate_lpfilt_sub_bline_DELAY_STOP__epoch_prePostProbe$block_1$baseline_cor_epochs[[4]]
...
#> 
#> $baseline_pupil_raw_deblink_detransient_interpolate_lpfilt_sub_bline_DELAY_STOP__epoch_prePostProbe$block_1$baseline_cor_epochs[[5]]
...
#> 
#> 
#> $baseline_pupil_raw_deblink_detransient_interpolate_lpfilt_sub_bline_DELAY_STOP__epoch_prePostProbe$block_1$baseline_means_by_epoch
#> [1] 6227.703 6287.244 5571.891 6094.619 6901.673
#> 
#> $baseline_pupil_raw_deblink_detransient_interpolate_lpfilt_sub_bline_DELAY_STOP__epoch_prePostProbe$block_1$baseline_correction_method
#> [1] "subtractive"
#> 
#> $baseline_pupil_raw_deblink_detransient_interpolate_lpfilt_sub_bline_DELAY_STOP__epoch_prePostProbe$block_1$baseline_cor_col_name
#> [1] "pupil_raw_deblink_detransient_interpolate_lpfilt_sub_bline_DELAY_STOP_"
#> 
#> $baseline_pupil_raw_deblink_detransient_interpolate_lpfilt_sub_bline_DELAY_STOP__epoch_prePostProbe$block_1$epoch_events
#> [1] "PROBE_START_{trial}"
#> 
#> $baseline_pupil_raw_deblink_detransient_interpolate_lpfilt_sub_bline_DELAY_STOP__epoch_prePostProbe$block_1$baseline_events
#> [1] "DELAY_STOP_*"
#> 
#> $baseline_pupil_raw_deblink_detransient_interpolate_lpfilt_sub_bline_DELAY_STOP__epoch_prePostProbe$block_1$n_epochs
#> [1] 5
#> 
#> $baseline_pupil_raw_deblink_detransient_interpolate_lpfilt_sub_bline_DELAY_STOP__epoch_prePostProbe$block_1$n_baseline_epochs
#> [1] 5
#> 
#> $baseline_pupil_raw_deblink_detransient_interpolate_lpfilt_sub_bline_DELAY_STOP__epoch_prePostProbe$block_1$info
#> $baseline_pupil_raw_deblink_detransient_interpolate_lpfilt_sub_bline_DELAY_STOP__epoch_prePostProbe$block_1$info$calc_baseline
#> [1] TRUE
#> 
#> $baseline_pupil_raw_deblink_detransient_interpolate_lpfilt_sub_bline_DELAY_STOP__epoch_prePostProbe$block_1$info$apply_baseline
#> [1] TRUE
#> 
#> $baseline_pupil_raw_deblink_detransient_interpolate_lpfilt_sub_bline_DELAY_STOP__epoch_prePostProbe$block_1$info$baseline_type
#> [1] "sub"
#> 
#> $baseline_pupil_raw_deblink_detransient_interpolate_lpfilt_sub_bline_DELAY_STOP__epoch_prePostProbe$block_1$info$baseline_events
#> [1] "DELAY_STOP_*"
#> 
#> $baseline_pupil_raw_deblink_detransient_interpolate_lpfilt_sub_bline_DELAY_STOP__epoch_prePostProbe$block_1$info$baseline_period
#> [1] -1  0
#> 
#> $baseline_pupil_raw_deblink_detransient_interpolate_lpfilt_sub_bline_DELAY_STOP__epoch_prePostProbe$block_1$info$epoch_events
#> [1] "PROBE_START_{trial}"
#> 
#> $baseline_pupil_raw_deblink_detransient_interpolate_lpfilt_sub_bline_DELAY_STOP__epoch_prePostProbe$block_1$info$epoch_limits
#> [1] 0 1
#> 
#> $baseline_pupil_raw_deblink_detransient_interpolate_lpfilt_sub_bline_DELAY_STOP__epoch_prePostProbe$block_1$info$n_epochs
#> [1] 5
#> 
#> $baseline_pupil_raw_deblink_detransient_interpolate_lpfilt_sub_bline_DELAY_STOP__epoch_prePostProbe$block_1$info$n_baseline_epochs
#> [1] 5
#> 
#> 
#> 
...
#> extracted
#> ✔ [2026-07-03 18:41:01] [OKAY] Block 1: 9 epochs baselined
#> ✔ [2026-07-03 18:41:01] [OKAY] Pupil epoching and baselining completed in 0.12
#> secs
#> ℹ [2026-07-03 18:41:01] [INFO] Recalculating epoched confounds for new
#> epochs...
#> $file
...
#> PROBE_START_22.pupil_raw_deblink_detransient_interpolate_lpfilt                            PROBE_START_22
#> PROBE_START_22.pupil_raw_deblink_detransient_interpolate_lpfilt_z                          PROBE_START_22
#> PROBE_START_22.pupil_raw_deblink_detransient_interpolate_lpfilt_sub_bline_multi_baseline   PROBE_START_22
#> PROBE_START_22.pupil_raw_deblink_detransient_interpolate_lpfilt_sub_bline_multi_baseline_z PROBE_START_22
#> PROBE_START_23.pupil_raw                                                                   PROBE_START_23
#> PROBE_START_23.pupil_raw_deblink                                                           PROBE_START_23
#> PROBE_START_23.pupil_raw_deblink_detransient                                               PROBE_START_23
#> PROBE_START_23.pupil_raw_deblink_detransient_interpolate                                   PROBE_START_23
#> PROBE_START_23.pupil_raw_deblink_detransient_interpolate_lpfilt                            PROBE_START_23
#> PROBE_START_23.pupil_raw_deblink_detransient_interpolate_lpfilt_z                          PROBE_START_23
#> PROBE_START_23.pupil_raw_deblink_detransient_interpolate_lpfilt_sub_bline_multi_baseline   PROBE_START_23
#> PROBE_START_23.pupil_raw_deblink_detransient_interpolate_lpfilt_sub_bline_multi_baseline_z PROBE_START_23
#> PROBE_START_24.pupil_raw                                                                   PROBE_START_24
#> PROBE_START_24.pupil_raw_deblink                                                           PROBE_START_24
#> PROBE_START_24.pupil_raw_deblink_detransient                                               PROBE_START_24
#> PROBE_START_24.pupil_raw_deblink_detransient_interpolate                                   PROBE_START_24
#> PROBE_START_24.pupil_raw_deblink_detransient_interpolate_lpfilt                            PROBE_START_24
#> PROBE_START_24.pupil_raw_deblink_detransient_interpolate_lpfilt_z                          PROBE_START_24
#> PROBE_START_24.pupil_raw_deblink_detransient_interpolate_lpfilt_sub_bline_multi_baseline   PROBE_START_24
#> PROBE_START_24.pupil_raw_deblink_detransient_interpolate_lpfilt_sub_bline_multi_baseline_z PROBE_START_24
#> PROBE_START_25.pupil_raw                                                                   PROBE_START_25
#> PROBE_START_25.pupil_raw_deblink                                                           PROBE_START_25
#> PROBE_START_25.pupil_raw_deblink_detransient                                               PROBE_START_25
#> PROBE_START_25.pupil_raw_deblink_detransient_interpolate                                   PROBE_START_25
#> PROBE_START_25.pupil_raw_deblink_detransient_interpolate_lpfilt                            PROBE_START_25
#> PROBE_START_25.pupil_raw_deblink_detransient_interpolate_lpfilt_z                          PROBE_START_25
#> PROBE_START_25.pupil_raw_deblink_detransient_interpolate_lpfilt_sub_bline_multi_baseline   PROBE_START_25
#> PROBE_START_25.pupil_raw_deblink_detransient_interpolate_lpfilt_sub_bline_multi_baseline_z PROBE_START_25
#> PROBE_START_26.pupil_raw                                                                   PROBE_START_26
#> PROBE_START_26.pupil_raw_deblink                                                           PROBE_START_26
#> PROBE_START_26.pupil_raw_deblink_detransient                                               PROBE_START_26
#> PROBE_START_26.pupil_raw_deblink_detransient_interpolate                                   PROBE_START_26
#> PROBE_START_26.pupil_raw_deblink_detransient_interpolate_lpfilt                            PROBE_START_26
#> PROBE_START_26.pupil_raw_deblink_detransient_interpolate_lpfilt_z                          PROBE_START_26
#> PROBE_START_26.pupil_raw_deblink_detransient_interpolate_lpfilt_sub_bline_multi_baseline   PROBE_START_26
#> PROBE_START_26.pupil_raw_deblink_detransient_interpolate_lpfilt_sub_bline_multi_baseline_z PROBE_START_26
#>                                                                                               text_unique
#> PROBE_START_22.pupil_raw                                                                   PROBE_START_22
#> PROBE_START_22.pupil_raw_deblink                                                           PROBE_START_22
#> PROBE_START_22.pupil_raw_deblink_detransient                                               PROBE_START_22
...
#> PROBE_START_22.pupil_raw_deblink_detransient_interpolate_lpfilt                                                       raw_deblink_detransient_interpolate_lpfilt
#> PROBE_START_22.pupil_raw_deblink_detransient_interpolate_lpfilt_z                                                   raw_deblink_detransient_interpolate_lpfilt_z
#> PROBE_START_22.pupil_raw_deblink_detransient_interpolate_lpfilt_sub_bline_multi_baseline     raw_deblink_detransient_interpolate_lpfilt_sub_bline_multi_baseline
#> PROBE_START_22.pupil_raw_deblink_detransient_interpolate_lpfilt_sub_bline_multi_baseline_z raw_deblink_detransient_interpolate_lpfilt_sub_bline_multi_baseline_z
#> PROBE_START_23.pupil_raw                                                                                                                                     raw
#> PROBE_START_23.pupil_raw_deblink                                                                                                                     raw_deblink
#> PROBE_START_23.pupil_raw_deblink_detransient                                                                                             raw_deblink_detransient
#> PROBE_START_23.pupil_raw_deblink_detransient_interpolate                                                                     raw_deblink_detransient_interpolate
#> PROBE_START_23.pupil_raw_deblink_detransient_interpolate_lpfilt                                                       raw_deblink_detransient_interpolate_lpfilt
#> PROBE_START_23.pupil_raw_deblink_detransient_interpolate_lpfilt_z                                                   raw_deblink_detransient_interpolate_lpfilt_z
#> PROBE_START_23.pupil_raw_deblink_detransient_interpolate_lpfilt_sub_bline_multi_baseline     raw_deblink_detransient_interpolate_lpfilt_sub_bline_multi_baseline
#> PROBE_START_23.pupil_raw_deblink_detransient_interpolate_lpfilt_sub_bline_multi_baseline_z raw_deblink_detransient_interpolate_lpfilt_sub_bline_multi_baseline_z
#> PROBE_START_24.pupil_raw                                                                                                                                     raw
#> PROBE_START_24.pupil_raw_deblink                                                                                                                     raw_deblink
#> PROBE_START_24.pupil_raw_deblink_detransient                                                                                             raw_deblink_detransient
#> PROBE_START_24.pupil_raw_deblink_detransient_interpolate                                                                     raw_deblink_detransient_interpolate
#> PROBE_START_24.pupil_raw_deblink_detransient_interpolate_lpfilt                                                       raw_deblink_detransient_interpolate_lpfilt
#> PROBE_START_24.pupil_raw_deblink_detransient_interpolate_lpfilt_z                                                   raw_deblink_detransient_interpolate_lpfilt_z
#> PROBE_START_24.pupil_raw_deblink_detransient_interpolate_lpfilt_sub_bline_multi_baseline     raw_deblink_detransient_interpolate_lpfilt_sub_bline_multi_baseline
#> PROBE_START_24.pupil_raw_deblink_detransient_interpolate_lpfilt_sub_bline_multi_baseline_z raw_deblink_detransient_interpolate_lpfilt_sub_bline_multi_baseline_z
#> PROBE_START_25.pupil_raw                                                                                                                                     raw
#> PROBE_START_25.pupil_raw_deblink                                                                                                                     raw_deblink
#> PROBE_START_25.pupil_raw_deblink_detransient                                                                                             raw_deblink_detransient
#> PROBE_START_25.pupil_raw_deblink_detransient_interpolate                                                                     raw_deblink_detransient_interpolate
#> PROBE_START_25.pupil_raw_deblink_detransient_interpolate_lpfilt                                                       raw_deblink_detransient_interpolate_lpfilt
#> PROBE_START_25.pupil_raw_deblink_detransient_interpolate_lpfilt_z                
1.713636
#> PROBE_START_25.pupil_raw_deblink_detransient                                                -1.713636
#> PROBE_START_25.pupil_raw_deblink_detransient_interpolate                                    -1.713636
#> PROBE_START_25.pupil_raw_deblink_detransient_interpolate_lpfilt                             -1.405919
#> PROBE_START_25.pupil_raw_deblink_detransient_interpolate_lpfilt_z                           -1.405919
#> PROBE_START_25.pupil_raw_deblink_detransient_interpolate_lpfilt_sub_bline_multi_baseline    -1.405919
#> PROBE_START_25.pupil_raw_deblink_detransient_interpolate_lpfilt_sub_bline_multi_baseline_z  -1.405919
#> PROBE_START_26.pupil_raw                                                                    -1.825580
#> PROBE_START_26.pupil_raw_deblink                                                            -1.825580
#> PROBE_START_26.pupil_raw_deblink_detransient                                                -1.825580
#> PROBE_START_26.pupil_raw_deblink_detransient_interpolate                                    -1.825580
#> PROBE_START_26.pupil_raw_deblink_detransient_interpolate_lpfilt                             -1.737069
#> PROBE_START_26.pupil_raw_deblink_detransient_interpolate_lpfilt_z                           -1.737069
#> PROBE_START_26.pupil_raw_deblink_detransient_interpolate_lpfilt_sub_bline_multi_baseline    -1.737069
#> PROBE_START_26.pupil_raw_deblink_detransient_interpolate_lpfilt_sub_bline_multi_baseline_z  -1.737069
#>                                                                                            prop_blink_time
#> PROBE_START_22.pupil_raw                                                                                 0
#> PROBE_START_22.pupil_raw_deblink                                                                         0
#> PROBE_START_22.pupil_raw_deblink_detransient                                                             0
...
#> PROBE_START_22.pupil_raw_deblink_detransient_interpolate_lpfilt                                   42.09339080
#> PROBE_START_22.pupil_raw_deblink_detransient_interpolate_lpfilt_z                                  0.09276402
#> PROBE_START_22.pupil_raw_deblink_detransient_interpolate_lpfilt_sub_bline_multi_baseline                   NA
#> PROBE_START_22.pupil_raw_deblink_detransient_interpolate_lpfilt_sub_bline_multi_baseline_z                 NA
#> PROBE_START_23.pupil_raw                                                                          41.04019932
#> PROBE_START_23.pupil_raw_deblink                                                                  41.04019932
#> PROBE_START_23.pupil_raw_deblink_detransient                                                      41.04019932
#> PROBE_START_23.pupil_raw_deblink_detransient_interpolate                                          41.04019932
#> PROBE_START_23.pupil_raw_deblink_detransient_interpolate_lpfilt                                   39.02897700
#> PROBE_START_23.pupil_raw_deblink_detransient_interpolate_lpfilt_z                                  0.08601077
#> PROBE_START_23.pupil_raw_deblink_detransient_interpolate_lpfilt_sub_bline_multi_baseline                   NA
#> PROBE_START_23.pupil_raw_deblink_detransient_interpolate_lpfilt_sub_bline_multi_baseline_z                 NA
#> PROBE_START_24.pupil_raw                                                                          62.93071686
#> PROBE_START_24.pupil_raw_deblink                                                                  62.93071686
#> PROBE_START_24.pupil_raw_deblink_detransient                                                      62.93071686
#> PROBE_START_24.pupil_raw_deblink_detransient_interpolate                                          62.93071686
#> PROBE_START_24.pupil_raw_deblink_detransient_interpolate_lpfilt                                   61.28090023
#> PROBE_START_24.pupil_raw_deblink_detransient_interpolate_lpfilt_z                                  0.13504882
#> PROBE_START_24.pupil_raw_deblink_detransient_interpolate_lpfilt_sub_bline_multi_baseline                   NA
#> PROBE_START_24.pupil_raw_deblink_detransient_interpolate_lpfilt_sub_bline_multi_baseline_z                 NA
#> PROBE_START_25.pupil_raw                                                                          40.62699823
#> PROBE_START_25.pupil_raw_deblink                                                                  40.62699823
#> PROBE_START_25.pupil_raw_deblink_detransient                                                      40.62699823
#> PROBE_START_25.pupil_raw_deblink_detransient_interpolate                                          40.62699823
#> PROBE_START_25.pupil_raw_deblink_detransient_interpolate_lpfilt                                   40.73405825
#> PROBE_START_25.pupil_raw_deblink_detransient_interpolate_lpfilt_z                                  0.08976837
#> PROBE_START_25.pupil_raw_deblink_detransient_interpolate_lpfilt_sub_bline_multi_baseline                   NA
#> PROBE_START_25.pupil_raw_deblink_detransient_interpolate_lpfilt_sub_bline_multi_baseline_z                 NA
#> PROBE_START_26.pupil_raw                                                                          20.27918326
#> PROBE_START_26.pupil_raw_deblink                                                                  20.27918326
#> PROBE_START_26.pupil_raw_deblink_detransient                                                      20.27918326
#> PROBE_START_26.pupil_raw_deblink_detransient_interpolate                                          20.27918326
#> PROBE_START_26.pupil_raw_deblink_detransient_interpolate_lpfilt                                   20.61066743
#> PROBE_START_26.pupil_raw_deblink_detransient_interpolate_lpfilt_z                                  0.04542111
#> PROBE_START_26.pupil_raw_deblink_detransient_interpolate_lpfilt_sub_bline_multi_baseline                   NA
#> PROBE_START_26.pupil_raw_deblink_detransient_interpolate_lpfilt_sub_bline_multi_baseline_z                 NA
#>                                                                                            epoch_pupil_sd
#> PROBE_START_22.pupil_raw                                                                      326.8312387
#> PROBE_START_22.pupil_raw_deblink                                                              326.8312387
#> PROBE_START_22.pupil_raw_deblink_detransient                                                  326.8312387
...
#> PROBE_START_22.pupil_raw_deblink_detransient_interpolate_lpfilt                               326.5297980
#> PROBE_START_22.pupil_raw_deblink_detransient_interpolate_lpfilt_z                               0.7195956
#> PROBE_START_22.pupil_raw_deblink_detransient_interpolate_lpfilt_sub_bline_multi_baseline      326.5297980
#> PROBE_START_22.pupil_raw_deblink_detransient_interpolate_lpfilt_sub_bline_multi_baseline_z      0.7295927
#> PROBE_START_23.pupil_raw                                                                      408.4668023
#> PROBE_START_23.pupil_raw_deblink                                                              408.4668023
#> PROBE_START_23.pupil_raw_deblink_detransient                                                  408.4668023
#> PROBE_START_23.pupil_raw_deblink_detransient_interpolate                                      408.4668023
#> PROBE_START_23.pupil_raw_deblink_detransient_interpolate_lpfilt                               408.3975024
#> PROBE_START_23.pupil_raw_deblink_detransient_interpolate_lpfilt_z                               0.9000129
#> PROBE_START_23.pupil_raw_deblink_detransient_interpolate_lpfilt_sub_bline_multi_baseline      408.3975024
#> PROBE_START_23.pupil_raw_deblink_detransient_interpolate_lpfilt_sub_bline_multi_baseline_z      0.9125165
#> PROBE_START_24.pupil_raw                                                                      277.3137048
#> PROBE_START_24.pupil_raw_deblink                                                              277.3137048
#> PROBE_START_24.pupil_raw_deblink_detransient                                                  277.3137048
#> PROBE_START_24.pupil_raw_deblink_detransient_interpolate                                      277.3137048
#> PROBE_START_24.pupil_raw_deblink_detransient_interpolate_lpfilt                               277.2579621
#> PROBE_START_24.pupil_raw_deblink_detransient_interpolate_lpfilt_z                               0.6110119
#> PROBE_START_24.pupil_raw_deblink_detransient_interpolate_lpfilt_sub_bline_multi_baseline      277.2579621
#> PROBE_START_24.pupil_raw_deblink_detransient_interpolate_lpfilt_sub_bline_multi_baseline_z      0.6195006
#> PROBE_START_25.pupil_raw                                                                      105.5142231
#> PROBE_START_25.pupil_raw_deblink                                                              105.5142231
#> PROBE_START_25.pupil_raw_deblink_detransient                                                  105.5142231
#> PROBE_START_25.pupil_raw_deblink_detransient_interpolate                                      105.5142231
#> PROBE_START_25.pupil_raw_deblink_detransient_interpolate_lpfilt                               105.0297599
#> PROBE_START_25.pupil_raw_deblink_detransient_interpolate_lpfilt_z                               0.2314611
#> PROBE_START_25.pupil_raw_deblink_detransient_interpolate_lpfilt_sub_bline_multi_baseline      105.0297599
#> PROBE_START_25.pupil_raw_deblink_detransient_interpolate_lpfilt_sub_bline_multi_baseline_z      0.2346767
#> PROBE_START_26.pupil_raw                                                                      357.3341023
#> PROBE_START_26.pupil_raw_deblink                                                              357.3341023
#> PROBE_START_26.pupil_raw_deblink_detransient                                                  357.3341023
#> PROBE_START_26.pupil_raw_deblink_detransient_interpolate                                      357.3341023
#> PROBE_START_26.pupil_raw_deblink_detransient_interpolate_lpfilt                               357.2021991
#> PROBE_START_26.pupil_raw_deblink_detransient_interpolate_lpfilt_z                               0.7871904
#> PROBE_START_26.pupil_raw_deblink_detransient_interpolate_lpfilt_sub_bline_multi_baseline      357.2021991
#> PROBE_START_26.pupil_raw_deblink_detransient_interpolate_lpfilt_sub_bline_multi_baseline_z      0.7981266
#> 
#> 
#> 
#> $confounds$epoched_epoch_wide
...
#> 
#> 
#> $baseline_pupil_raw_deblink_detransient_interpolate_lpfilt_sub_bline_multi_baseline_epoch_prePostProbe
#> $baseline_pupil_raw_deblink_detransient_interpolate_lpfilt_sub_bline_multi_baseline_epoch_prePostProbe$block_1
#> $baseline_pupil_raw_deblink_detransient_interpolate_lpfilt_sub_bline_multi_baseline_epoch_prePostProbe$block_1$baseline_cor_epochs
#> $baseline_pupil_raw_deblink_detransient_interpolate_lpfilt_sub_bline_multi_baseline_epoch_prePostProbe$block_1$baseline_cor_epochs[[1]]
...
#> 
#> $baseline_pupil_raw_deblink_detransient_interpolate_lpfilt_sub_bline_multi_baseline_epoch_prePostProbe$block_1$baseline_cor_epochs[[2]]
...
#> 
#> $baseline_pupil_raw_deblink_detransient_interpolate_lpfilt_sub_bline_multi_baseline_epoch_prePostProbe$block_1$baseline_cor_epochs[[3]]
...
#> 
#> $baseline_pupil_raw_deblink_detransient_interpolate_lpfilt_sub_bline_multi_baseline_epoch_prePostProbe$block_1$baseline_cor_epochs[[4]]
...
#> 
#> $baseline_pupil_raw_deblink_detransient_interpolate_lpfilt_sub_bline_multi_baseline_epoch_prePostProbe$block_1$baseline_cor_epochs[[5]]
...
#> 
#> 
#> $baseline_pupil_raw_deblink_detransient_interpolate_lpfilt_sub_bline_multi_baseline_epoch_prePostProbe$block_1$baseline_means_by_epoch
#> [1] 6227.105 6286.692 5571.269 6094.719 6900.015
#> 
#> $baseline_pupil_raw_deblink_detransient_interpolate_lpfilt_sub_bline_multi_baseline_epoch_prePostProbe$block_1$baseline_correction_method
#> [1] "subtractive"
#> 
#> $baseline_pupil_raw_deblink_detransient_interpolate_lpfilt_sub_bline_multi_baseline_epoch_prePostProbe$block_1$baseline_cor_col_name
#> [1] "pupil_raw_deblink_detransient_interpolate_lpfilt_sub_bline_multi_baseline"
#> 
#> $baseline_pupil_raw_deblink_detransient_interpolate_lpfilt_sub_bline_multi_baseline_epoch_prePostProbe$block_1$epoch_events
#> [1] "PROBE_START_{trial}"
#> 
#> $baseline_pupil_raw_deblink_detransient_interpolate_lpfilt_sub_bline_multi_baseline_epoch_prePostProbe$block_1$baseline_events
#> [1] "DELAY_START_*" "DELAY_STOP_*" 
#> 
#> $baseline_pupil_raw_deblink_detransient_interpolate_lpfilt_sub_bline_multi_baseline_epoch_prePostProbe$block_1$n_epochs
#> [1] 5
#> 
#> $baseline_pupil_raw_deblink_detransient_interpolate_lpfilt_sub_bline_multi_baseline_epoch_prePostProbe$block_1$n_baseline_epochs
#> [1] 5
#> 
#> $baseline_pupil_raw_deblink_detransient_interpolate_lpfilt_sub_bline_multi_baseline_epoch_prePostProbe$block_1$info
#> $baseline_pupil_raw_deblink_detransient_interpolate_lpfilt_sub_bline_multi_baseline_epoch_prePostProbe$block_1$info$calc_baseline
#> [1] TRUE
#> 
#> $baseline_pupil_raw_deblink_detransient_interpolate_lpfilt_sub_bline_multi_baseline_epoch_prePostProbe$block_1$info$apply_baseline
#> [1] TRUE
#> 
#> $baseline_pupil_raw_deblink_detransient_interpolate_lpfilt_sub_bline_multi_baseline_epoch_prePostProbe$block_1$info$baseline_type
#> [1] "sub"
#> 
#> $baseline_pupil_raw_deblink_detransient_interpolate_lpfilt_sub_bline_multi_baseline_epoch_prePostProbe$block_1$info$baseline_events
#> [1] "DELAY_START_*" "DELAY_STOP_*" 
#> 
#> $baseline_pupil_raw_deblink_detransient_interpolate_lpfilt_sub_bline_multi_baseline_epoch_prePostProbe$block_1$info$baseline_period
#> NULL
#> 
#> $baseline_pupil_raw_deblink_detransient_interpolate_lpfilt_sub_bline_multi_baseline_epoch_prePostProbe$block_1$info$epoch_events
#> [1] "PROBE_START_{trial}"
#> 
#> $baseline_pupil_raw_deblink_detransient_interpolate_lpfilt_sub_bline_multi_baseline_epoch_prePostProbe$block_1$info$epoch_limits
#> [1] 0 1
#> 
#> $baseline_pupil_raw_deblink_detransient_interpolate_lpfilt_sub_bline_multi_baseline_epoch_prePostProbe$block_1$info$n_epochs
#> [1] 5
#> 
#> $baseline_pupil_raw_deblink_detransient_interpolate_lpfilt_sub_bline_multi_baseline_epoch_prePostProbe$block_1$info$n_baseline_epochs
#> [1] 5
#> 
#> 
#> 
...
#> ℹ [2026-07-03 18:41:01] [INFO] Epoching pupil data...
#> ! [2026-07-03 18:41:01] [WARN] Manual epoching only works with 1 block at a
#> time. Manual epoch input must be a list of 2 data frames and 1 numeric:
#> `start_events` (df), `end_events` (df), and `block` (numeric). Please be sure
#> to explicitly indicate the block number in your input list! (see example #9 in
#> the documentation for more details).
#> ✔ [2026-07-03 18:41:01] [OKAY] Done!
#> ✔ [2026-07-03 18:41:01] [OKAY] Block 1: pupil data from 2 unique event messages
#> extracted
#> ✔ [2026-07-03 18:41:01] [OKAY] Pupil epoching completed in 0.03 seconds
#> ℹ [2026-07-03 18:41:01] [INFO] Recalculating epoched confounds for new
#> epochs...
#> $file
#> [1] "/home/runner/work/_temp/Library/eyeris/extdata/memory.asc"
...
#> $params$epoch
#> $params$epoch$call_stack
#> eyeris::epoch(eyeris = eye_preproc, events = list(start_events, 
#>     end_events, block_number), label = "example9")
#> 
#> $params$epoch$parameters
#> $params$epoch$parameters$events
#> $params$epoch$parameters$events[[1]]
#>       time        msg
#> 1 11334491 TRIALID 22
#> 2 11338691 TRIALID 23
#> 
#> $params$epoch$parameters$events[[2]]
#>       time         msg
#> 1 11337158 RESPONSE_22
#> 2 11341292 RESPONSE_23
#> 
#> $params$epoch$parameters$events[[3]]
#> [1] 1
#> 
...
#> 
#> $confounds$epoched_timeseries
#> $confounds$epoched_timeseries$epoch_example9
#> list()
#> 
#> 
#> $confounds$epoched_epoch_wide
#> $confounds$epoched_epoch_wide$epoch_example9
#> list()
#> 
#> 
#> 
...
#> 
#> $epoch_example9$info
#> $epoch_example9$info$block_1
#> $epoch_example9$info$block_1$calc_baseline
#> [1] FALSE
#> 
#> $epoch_example9$info$block_1$apply_baseline
#> [1] FALSE
#> 
#> $epoch_example9$info$block_1$epoch_events
#> $epoch_example9$info$block_1$epoch_events[[1]]
#>       time        msg
#> 1 11334491 TRIALID 22
#> 2 11338691 TRIALID 23
#> 
#> $epoch_example9$info$block_1$epoch_events[[2]]
#>       time         msg
#> 1 11337158 RESPONSE_22
#> 2 11341292 RESPONSE_23
#> 
#> $epoch_example9$info$block_1$epoch_events[[3]]
#> [1] 1
#> 
#> 
#> $epoch_example9$info$block_1$epoch_limits
#> NULL
#> 
#> $epoch_example9$info$block_1$n_epochs
#> [1] 2
#> 
#> 
#> 
